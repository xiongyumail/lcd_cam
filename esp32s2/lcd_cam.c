...
static void IRAM_ATTR spi_isr(void *arg)
{
    int event;
    BaseType_t HPTaskAwoken = pdFALSE;
    typeof(GPSPI3.dma_int_st) status = GPSPI3.dma_int_st;
    GPSPI3.dma_int_clr.val = status.val;
    if (status.val == 0) {
        return;
    }
...
static void i2s_write_data(uint8_t *data, size_t len)
{
    ...
    if (left) {
        ...
        xQueueReceive(lcd_cam_obj->lcd.event_queue, (void *)&event, portMAX_DELAY);
        i2s_start(((uint32_t)&lcd_cam_obj->lcd.dma[(x % 2) * lcd_cam_obj->lcd.dma_half_node_cnt]) & 0xfffff, left);
    }
    xQueueReceive(lcd_cam_obj->lcd.event_queue, (void *)&event, portMAX_DELAY);
    while (!I2S0.state.tx_idle);
}
...
static esp_err_t lcd_set_pin(lcd_config_t *config)
{
    if (config->width == 1) {
        ...
        gpio_matrix_out(config->pin.clk, SPI3_CLK_OUT_MUX_IDX, 0, 0);
        ...
        gpio_matrix_out(config->pin.data[0], SPI3_D_OUT_IDX, config->invert.data[0], 0);
        ...
        for(int i = 0; i < config->width; i++) {
            ...
            // fifo accesses data by bit, when tx_bits_mod is 8, the data needs to be aligned by 8 bits
...
static esp_err_t cam_dma_config(cam_config_t *config) 
{
    ...
    I2S0.rx_eof_num = lcd_cam_obj->cam.dma_half_buffer_size; // Ping pong operation
    return ESP_OK;
}
...
esp_err_t lcd_cam_init(lcd_cam_handle_t *handle, lcd_cam_config_t *config)
{
    ...
    if (config->lcd.width == 1) {
        if (REG_GET_BIT(DPORT_PERIP_CLK_EN0_REG, DPORT_SPI3_CLK_EN) == 0) {
    ...
    if (lcd_cam_obj->lcd_en) {
        ...
        if (config->lcd.width == 1) {
            ...
            handle->lcd.write_data = i2s_write_data;
        }
